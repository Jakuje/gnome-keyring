...
#define ed25519_BYTES 64U

CK_RV
gkm_eddsa_mechanism_sign (gcry_sexp_t sexp, CK_BYTE_PTR data, CK_ULONG n_data,
                        CK_BYTE_PTR signature, CK_ULONG_PTR n_signature)
{
	...
	if (!signature) {
		*n_signature = ed25519_BYTES;
		return CKR_OK;
	} else if (*n_signature < ed25519_BYTES) {
		*n_signature = ed25519_BYTES;
		return CKR_BUFFER_TOO_SMALL;
	}

	/* Prepare the input s-expression */
	gcry = gcry_sexp_build (&splain, NULL, "(data (flags eddsa) (hash-algo sha512) (value %b))", n_data, data);
	...
	g_assert (*n_signature >= ed25519_BYTES);

	size = 32;
	rv = gkm_crypto_sexp_to_buffer (ssig, signature, &size, NULL, "eddsa", "r", NULL);
	if (rv == CKR_OK) {
		g_return_val_if_fail (size == 32, CKR_GENERAL_ERROR);
		rv = gkm_crypto_sexp_to_buffer (ssig, signature + 32, &size, NULL, "eddsa", "s", NULL);
		if (rv == CKR_OK) {
			g_return_val_if_fail (size == 32, CKR_GENERAL_ERROR);
			*n_signature = ed25519_BYTES;
		}
...
CK_RV
gkm_eddsa_mechanism_verify (gcry_sexp_t sexp, CK_BYTE_PTR data, CK_ULONG n_data,
                          CK_BYTE_PTR signature, CK_ULONG n_signature)
{
	...
	gcry = gcry_sexp_build (&ssig, NULL, "(sig-val (eddsa (r %b) (s %b)))", 32, signature, 32, signature + 32);
	g_return_val_if_fail (gcry == 0, CKR_GENERAL_ERROR);
