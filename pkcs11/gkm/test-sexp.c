                       ...
                       CK_ATTRIBUTE_PTR attrs, CK_ULONG_PTR n_attrs)
{
	...
	attr = &attrs[(*n_attrs)++];
	attr->pValue = (void *) malloc (sizeof (gchar)*8);
	attr->ulValueLen = 8;
                       ...
                       CK_ATTRIBUTE_PTR attrs, CK_ULONG_PTR n_attrs)
{
	...
	attr = &attrs[(*n_attrs)++];
	attr->pValue = (void *) malloc (sizeof (gchar));
	attr->ulValueLen = 1;
...
static void
assert_get_attribute_buffer (GkmPrivateXsaKey *key, CK_ATTRIBUTE_TYPE type, char *exp, gsize exp_len,
                         CK_ATTRIBUTE_PTR attrs, CK_ULONG_PTR n_attrs)
{
	CK_ATTRIBUTE_PTR attr;
	CK_RV ret;
	...
	attr = &attrs[(*n_attrs)++];
	attr->pValue = (void *) malloc (sizeof (gchar)*512);
	attr->ulValueLen = 512;
...
static void
assert_sexp_compare_mpi (gcry_sexp_t s1, gcry_sexp_t s2, gchar *field)
{
	gcry_mpi_t m1, m2;
...
static void
assert_sexp_compare_bytes (gcry_sexp_t s1, gcry_sexp_t s2, gchar *field)
{
	gchar *b1, *b2;
	gsize bs1, bs2;
